<script setup lang="ts">
    ...
    async function search() {
        if (searchStore.query?.length) {
            loading.value = true;
            try {
                const response = await fetch(`${API_URL}search/shows?q=${searchStore.query}`);
...
<template>
    <div class="tv-shows">
      ...
      <div v-if="tvShows.length > 0" class="tv-show-list">
        <img v-for="tvShow in tvShows" :key="tvShow.id" :src="tvShow.image?.medium" alt="TV Show">
      </div>
      <div v-if="!pristine && !tvShows.length" class="no-results">No results found</div>
